...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    [Header("Grapple Hook")]
    [SerializeField] private Transform grappleWire = null;
    [SerializeField] private float normalFOV = 60f;
    [SerializeField] private float swingingFOV = 100f;
    [SerializeField] private float grappleSpeedMin = 20f;
    [SerializeField] private float grappleSpeedMax = 70f;
    [SerializeField] private float length = 20f;
    [SerializeField] private float minGrapplePosDistance = 1f;
    [SerializeField] private float grappleSpeedMultiplier = 2f;
    [SerializeField] private float grappleThrowSpeed = 20f;
    [SerializeField] private float grappleSize = 1f;
    private State state;
    ...
    void Start()
    {
        ...
        state = State.normal;
        grappleWire.gameObject.SetActive(false);
    }
    ...
    void GrappleStart()
    {
        ...
        {
            if (Physics.Raycast(playerCamera.transform.position, playerCamera.transform.forward, out RaycastHit rayCastHit/*, length*/))
            {
                if (rayCastHit.transform.gameObject.CompareTag("Hookable"))
                {
                    grapplePoint = rayCastHit.point;
                    grappleSize = 0f;
                    grappleWire.gameObject.SetActive(true);
                    grappleWire.localScale = Vector3.zero;
    ...
    {
        grappleWire.LookAt(grapplePoint);
        ...
        isGrounded = false;
        characterVelocity += Vector3.up * jumpForce;
        m_Controller.Move(grappleDir * grappleSpeed * grappleSpeedMultiplier * Time.deltaTime);

        if(Vector3.Distance(transform.position, grapplePoint) <= minGrapplePosDistance)
        {
            state = State.normal;
            ResetGravity();
            grappleWire.gameObject.SetActive(false);
        ...
        //{
        //    momentum = grappleDir * grappleSpeed * momentumSpeedMultiplier;
        //    state = State.normal;
        //    ResetGravity();
        //}
    }
    ...
    void GrappleThrown()
    {
        ...
        grappleWire.localScale = new Vector3(0.02f, 0.02f, grappleSize);

        if(grappleSize >= Vector3.Distance(transform.position, grapplePoint))
        {
            state = State.swinging;
            //playerCamera.fieldOfView = swingingFOV;
        }
    }
