...
public class CrosshairManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        UpdateCrossHairPointingAtHookable(false);
        m_WasPointingAtEnemy = m_WeaponsManager.isPointingAtEnemy;
        m_WasPointingAtHookable = m_WeaponsManager.isPointingAtHookable;
    }
    ...
    void UpdateCrossHairPointingAtHookable(bool force)
    {
        if ((force || !m_WasPointingAtHookable) && m_WeaponsManager.isPointingAtHookable)
        {
            Debug.Log("IS POINTING AT HOOKABLE");
            m_CurrentCrosshair = m_CrosshairDataGrapple;
            crosshairImage.sprite = m_CurrentCrosshair.crosshairSprite;
            //m_CrosshairRectTransform.sizeDelta = m_CurrentCrosshair.crosshairSize * Vector2.one;
        }
        else if ((force || m_WasPointingAtHookable) && !m_WeaponsManager.isPointingAtHookable)
        {
            m_CurrentCrosshair = m_CrosshairDataDefault;
            crosshairImage.sprite = m_CurrentCrosshair.crosshairSprite;
            //m_CrosshairRectTransform.sizeDelta = m_CurrentCrosshair.crosshairSize * Vector2.one;
    ...
    void OnWeaponChanged(WeaponController newWeapon)
    {
        if(newWeapon)
        {
            ...
            m_CrosshairDataGrapple = newWeapon.crosshairDataHookableInSight;
        ...
        UpdateCrossHairPointingAtHookable(true);
    }
}
